...
do
    if [[ ${!library} -eq 1 ]]; then
        ...
        case $ENABLED_LIBRARY in
            ...
            ;;
            android-media-codec)
                CONFIGURE_POSTFIX+=" --enable-mediacodec --enable-static --enable-hwaccels --enable-jni --enable-mediacodec --enable-decoder=hevc_mediacodec"
            ;;
        esac
    else
...
./configure \
    ...
    --enable-shared \
    --enable-gpl \
    --enable-filter=gltransition,glcomposite,glframes \
    --enable-decoder=png \
    --enable-encoder=png \
    --enable-decoder=gif \
    --enable-encoder=gif \
